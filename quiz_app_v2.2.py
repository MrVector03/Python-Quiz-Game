...
Introducing Manual Mode. Manual is basically some type of eraser for all the mistakes created
in the database, but it also holds an ability to control the database. As of now it's for
developers only but later these features will also be implemented into the program (with some
modifications) to be user friendly and maximize features.
"""
...
def create_questions():
    ...
    if quiz_list[0][5] == 1:
        ...
        db.commit()
        print("\nQuestions created")
        cursor.execute("UPDATE Quiz SET has_questions = 1 WHERE has_questions = s%", (quiz_list[0][0],))
        db.commit()
        print("\ncommit successful...")
...
"""
Manual mode will be created below in order to manually fix some of the errors
in the database created by skipping some database commits due to minor execution
bugs. These controls won't be used by users, this part of the program is created 
just for development and bug fix purposes only.
"""
